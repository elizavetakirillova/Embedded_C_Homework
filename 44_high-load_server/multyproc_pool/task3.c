 ...
 * с обслуживающими серверами.
 * Слушающий сервер следит за пулом обслуживающих серверов
 * и при подключении клиента идентификатор свободного сервера возвращает клиенту,
 * клиент подключается к нему.
 ...
 * (только для потоков - слушающий сервер будет искать в массиве 0 и перераспределять на него нагрузку),
 * канал, очереди сообщений, сигналы.
  ...
  while(!program_exit_flag){
    ...
    if(message_ext.msg.type == MESSAGE_CONNECT) {
      ...
      if (available_service_id != -1) {
        ...
        // N.B. в идеале каждый сервис должен работать на своем порту,
        // а слушающий сервер должен раздавать команды на подключение и отключение клиентов
        // но здесь мы просто организуем работу сервисов через IPC очереди как написано в задании
